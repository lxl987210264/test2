...
if(leftcircle.attr("cx")!=undefined){
	...
	if(leftline1.size()>1){
	   for(var i=0;i<leftline1.size();i++){
	     leftline=leftline1.get(i);
	   leftline.setAttribute("x1",d3.event.x/1-0.1);
	   leftline.setAttribute("y1",d3.event.y+obj.attr("height")/2-0.1);
	...
	}else if(leftline1.size()==1){
	   leftline=leftline1;
	   leftline.attr("x1",d3.event.x/1-0.1);
	   leftline.attr("y1",d3.event.y+obj.attr("height")/2-0.1);
	   $("circle[cx='"+(leftcircle.attr("cx")/1-0.1)+"'][cy='"+(leftcircle.attr("cy")/1-0.1)+"']").attr("cx",d3.event.x/1-0.1).attr("cy",d3.event.y+obj.attr("height")/2-0.1);
	  
	}
	if(leftline2.size()>1){
	 for(var i=0;i<leftline2.size();i++){
	   leftline=leftline2.get(i);
	  leftline.setAttribute("x2",d3.event.x/1-0.1);
	  leftline.setAttribute("y2",d3.event.y/1+obj.attr("height")/2-0.1);
	   $("circle[cx='"+(leftcircle.attr("cx")/1-0.1)+"'][cy='"+(leftcircle.attr("cy")/1-0.1)+"']").attr("cx",d3.event.x/1-0.1).attr("cy",d3.event.y+obj.attr("height")/2-0.1);
	
	 }
	
	}else if(leftline2.size()==1){
	 leftline=leftline2;
...
function rightcircleline(obj){
	...
	if(rightline1.size()>1){
	  for(var i=0;i<rightline1.size();i++){
	  rightline=rightline1.get(i);
	   rightline.setAttribute("x1",d3.event.x+obj.attr("width")/1-0.1);
	   rightline.setAttribute("y1",d3.event.y+obj.attr("height")/2-0.1);
	 ...
	 }else if(rightline1.size()==1){
	  rightline=rightline1;
	   rightline.attr("x1",d3.event.x+obj.attr("width")/1-0.1).attr("y1",d3.event.y+obj.attr("height")/2-0.1);
	   $("circle[cx='"+(rightcircle.attr("cx")/1-0.1)+"'][cy='"+(rightcircle.attr("cy")/1-0.1)+"']").attr("cx",d3.event.x+obj.attr("width")/1-0.1).attr("cy",d3.event.y+obj.attr("height")/2-0.1);
	
	 }
	 if(rightline2.size()>1){
	  for(var i=0;i<rightline2.size();i++){
	     rightline=rightline2.get(i);
	     rightline.setAttribute("x2",d3.event.x+obj.attr("width")/1-0.1);
	     rightline.setAttribute("y2",d3.event.y+obj.attr("height")/2-0.1);
	     $("circle[cx='"+(rightcircle.attr("cx")/1-0.1)+"'][cy='"+(rightcircle.attr("cy")/1-0.1)+"']").attr("cx",d3.event.x+obj.attr("width")/1-0.1).attr("cy",d3.event.y+obj.attr("height")/2-0.1);
	  }
	 
	 }else if(rightline2.size()==1){
...
function buttomcircleline(obj){
 if(bottomcircle.attr("cx")!=undefined){
	...
	if(bottomline1.size()>1){
	  for(var i=0;i<bottomline1.size();i++){
	      bottomline=bottomline1.get(i);
	   bottomline.setAttribute("x1",d3.event.x+obj.attr("width")/2-0.1);
	   bottomline.setAttribute("y1",d3.event.y+obj.attr("height")/1-0.1);
	...
	}else if(bottomline1.size()==1){
	   bottomline=bottomline1;
	   bottomline.attr("x1",d3.event.x+obj.attr("width")/2-0.1).attr("y1",d3.event.y+obj.attr("height")/1-0.1);
	   $("circle[cx='"+(bottomcircle.attr("cx")/1-0.1)+"'][cy='"+(bottomcircle.attr("cy")/1-0.1)+"']").attr("cx",d3.event.x+obj.attr("width")/2-0.1).attr("cy",d3.event.y+obj.attr("height")/1-0.1);
	
	}
	if(bottomline2.size()>1){
	 for(var i=0;i<bottomline2.size();i++){
	    bottomline=bottomline2.get(i);
	  bottomline.setAttribute("x2",d3.event.x+obj.attr("width")/2-0.1);
	  bottomline.setAttribute("y2",d3.event.y/1+obj.attr("height")/1-0.1);
	   $("circle[cx='"+(bottomcircle.attr("cx")/1-0.1)+"'][cy='"+(bottomcircle.attr("cy")/1-0.1)+"']").attr("cx",d3.event.x+obj.attr("width")/2-0.1).attr("cy",d3.event.y+obj.attr("height")/1-0.1);
	
	 }
	}else if(bottomline2.size()==1){
...
if(leftcircle!=null){
leftcircle.attr("display","none");
}
if(topcircle!=null){topcircle.attr("display","none");}
if(rightcircle!=null){rightcircle.attr("display","none");}
if(bottomcircle!=null){bottomcircle.attr("display","none");}
leftcircle=null;
topcircle=null;
rightcircle=null;
bottomcircle=null;
leftcircle=$("circle[cx='"+d3.select(this).attr("x")/1+"'][cy='"+(d3.select(this).attr("y")/1+d3.select(this).attr("height")/2)+"']");
leftcircle.attr("display","block");
topcircle=$("circle[cx='"+(d3.select(this).attr("x")/1+d3.select(this).attr("width")/2)+"'][cy='"+d3.select(this).attr("y")/1+"']");
topcircle.attr("display","block");
...
rightcircle.attr("display","block");
...
bottomcircle.attr("display","block");
}
//拖动结束事件
...
//方块左边的圆点
	 svg.append("circle")
			...
			.attr("cy",currect.attr("y")/1+currect.attr("height")/2)
			 .attr("r",3)
	        .attr("stroke","blue")
			.attr("fill","white").attr("display","none")
			.call(dragrectcircle);
//方块上边的圆点
	 svg.append("circle")
			.attr("cx",currect.attr("x")/1+currect.attr("width")/2)
			.attr("cy",currect.attr("y")/1).attr("display","none")
			.attr("r",3).attr("stroke","blue")
			.attr("fill","white");
//方块右边的圆点			
	 svg.append("circle")
			.attr("cx",currect.attr("x")/1+currect.attr("width")/1)
			.attr("cy",currect.attr("y")/1+currect.attr("height")/2)
			.attr("r",3).attr("stroke","blue").attr("display","none")
   ...
   d3.select(this).attr("cx",d3.event.x);
    if(topcircle.attr("cx")!=undefined){
		...
		if(topline1.size()>1){
		  for(var i=0;i<topline1.size();i++){
		    topline=topline1.get(i);
		    var line1=$("line[x1='"+topline.getAttribute("x2")+"'][y1='"+topline.getAttribute("y2")+"']");
		   topline.setAttribute("x1",d3.event.x+currect.getAttribute("width")/2-0.1);
		   topline.setAttribute("y1",currect.getAttribute("y")/1-0.1);
		   if(line1.size()>0){
		  if(line1.attr("y1")/1!=topline.getAttribute("y1")/1){
		    topline.setAttribute("x2",currect.getAttribute("x")/1+currect.getAttribute("width")/2-0.1);
		    line1.attr("x1",currect.getAttribute("x")/1+currect.getAttribute("width")/2-0.1);
		   }
		  }
		...
		topline=topline1;
		var line1=$("line[x1='"+topline.attr("x2")+"'][y1='"+topline.attr("y2")+"']");
		   if(line1.size()>0){
		      if(line1.attr("y1")/1!=topline.attr("y1")/1){
		      topline.attr("x2",currect.getAttribute("x")/1+currect.getAttribute("width")/2-0.1);
		      line1.attr("x1",currect.getAttribute("x")/1+currect.getAttribute("width")/2-0.1);
		   }
		   }
		...
		if(topline2.size()>1){
		 for(var i=0;i<topline2.size();i++){
		     topline=topline2.get(i);
		     var line1=$("line[x2='"+topline.getAttribute("x1")+"'][y2='"+topline.getAttribute("y1")+"']");
		  topline.setAttribute("x2",d3.event.x+currect.getAttribute("width")/2-0.1);
		  topline.setAttribute("y2",currect.getAttribute("y")/1-0.1);
		  if(line1.attr("y1")/1==topline.getAttribute("y1")/1){
		    topline.setAttribute("x1",currect.getAttribute("x")/1+currect.getAttribute("width")/2-0.1);
		    line1.attr("x2",currect.getAttribute("x")/1+currect.getAttribute("width")/2-0.1);
		   }
		   $("circle[cx='"+(topcircle.attr("cx")/1-0.1)+"'][cy='"+(topcircle.attr("cy")/1-0.1)+"']").attr("cx",d3.event.x+currect.getAttribute("width")/2-0.1).attr("cy",currect.getAttribute("y")/1-0.1);
		   }
		}else if(topline2.size()==1){
		 topline=topline2;
		  var line1=$("line[x2='"+topline.attr("x1")+"'][y2='"+topline.attr("y1")+"']");
		 if(line1.attr("y1")/1==topline.attr("y1")/1){
		    topline.attr("x1",currect.getAttribute("x")/1+currect.getAttribute("width")/2-0.1);
		    line1.attr("x2",currect.getAttribute("x")/1+currect.getAttribute("width")/2-0.1);
		   }
	 ...
	 if(bottomcircle.attr("cx")!=undefined){
		...
		if(bottomline1.size()>1){
		  for(var i=0;i<bottomline1.size();i++){
		     bottomline=bottomline1.get(i);
		      var line1=$("line[x1='"+bottomline.getAttribute("x2")+"'][y1='"+bottomline.getAttribute("y2")+"']");
		   bottomline.setAttribute("x1",d3.event.x+currect.getAttribute("width")/2-0.1);
		   bottomline.setAttribute("y1",currect.getAttribute("y")/1+currect.getAttribute("height")/1-0.1);
		    if(line1.size()>0){
		      if(line1.attr("y1")/1!=bottomline.getAttribute("y1")){
		    bottomline.setAttribute("x2",currect.getAttribute("x")/1+currect.getAttribute("width")/2-0.1);
		    line1.attr("x1",currect.getAttribute("x")/1+currect.getAttribute("width")/2-0.1);
		    }
		    }
		  ...
		  bottomline=bottomline1;
		   if(line1.size()>0){
		    if(line1.attr("y1")/1!=bottomline.attr("y1")){
		    bottomline.attr("x2",currect.getAttribute("x")/1+currect.getAttribute("width")/2-0.1);
		    line1.attr("x1",currect.getAttribute("x")/1+currect.getAttribute("width")/2-0.1);
		    }
		   }
		...
		if(bottomline2.size()>1){
		  for(var i=0;i<bottomline2.size();i++){
		    bottomline=bottomline2.get(i);
		     var line1=$("line[x2='"+bottomline.getAttribute("x1")+"'][y2='"+bottomline.getAttribute("y1")+"']");
		    bottomline.setAttribute("x2",d3.event.x+currect.getAttribute("width")/2-0.1);
		    bottomline.setAttribute("y2",currect.getAttribute("y")/1+currect.getAttribute("height")/1-0.1);
		     if(line1.attr("y2")/1!=bottomline.getAttribute("y2")){
		      bottomline.setAttribute("x1",(currect.getAttribute("x")/1+currect.getAttribute("width")/2-0.1));
		      line1.attr("x2",(currect.getAttribute("x")/1+currect.getAttribute("width")/2-0.1));
		   }
		    $("circle[cx='"+(bottomcircle.attr("cx")/1-0.1)+"'][cy='"+(bottomcircle.attr("cy")/1-0.1)+"']").attr("cx",d3.event.x+currect.getAttribute("width")/2-0.1).attr("cy",currect.getAttribute("y")/1+currect.getAttribute("height")/1-0.1);
		  }
		   ...
		   bottomline=bottomline2;
		    var line1=$("line[x2='"+bottomline.attr("x1")+"'][y2='"+bottomline.attr("y1")+"']");
		    if(line1.attr("y2")/1!=bottomline.attr("y2")){
		       bottomline.attr("x1",(currect.getAttribute("x")/1+currect.getAttribute("width")/2-0.1));
		       line1.attr("x2",(currect.getAttribute("x")/1+currect.getAttribute("width")/2-0.1));
		    }
	...
	if(leftcircle.attr("cx")!=undefined){
	   ...
	   for(var i=0;i<leftline1.size();i++){
	      leftline=leftline1.get(i);
		   leftline.setAttribute("x1",d3.event.x/1-0.1);
		   leftline.setAttribute("y1",currect.getAttribute("y")/1+currect.getAttribute("height")/2-0.1);
	...
	if(leftline2.size()>1){
	 for(var i=0;i<leftline2.size();i++){
	      leftline=leftline2.get(i);
		  leftline.setAttribute("x2",d3.event.x/1-0.1);
		  leftline.setAttribute("y2",currect.getAttribute("y")/1+currect.getAttribute("height")/2-0.1);
		  $("circle[cx='"+(leftcircle.attr("cx")/1-0.1)+"'][cy='"+(leftcircle.attr("cy")/1-0.1)+"']").attr("cx",d3.event.x/1-0.1).attr("cy",currect.getAttribute("y")/1+currect.getAttribute("height")/2-0.1);
	 }
  ...
  if(currect.getAttribute("width")/1+(d3.event.x/1-currect.getAttribute("x")/1-oldwidth)>10){
    ...
    if(topcircle.attr("cx")!=undefined){
	 ...
	 if(bottomcircle.attr("cx")!=undefined){
		...
		if(bottomline1.size()>1){
		  ...
		  bottomline=bottomline1;
		   var line1=$("line[x1='"+bottomline.attr("x2")+"'][y1='"+bottomline.attr("y2")+"']");
		   if(line1.size()>0){
		    if(line1.attr("y1")/1!=bottomline.attr("y1")){
		    bottomline.attr("x2",currect.getAttribute("x")/1+currect.getAttribute("width")/2-0.1);
		    line1.attr("x1",currect.getAttribute("x")/1+currect.getAttribute("width")/2-0.1);
   ...
   if(rightcircle.attr("cx")!=undefined){
		...
		if(rightline1.size()>1){
		   for(var i=0;i<rightline1.size();i++){
		   rightline=rightline1.get(i);
		   rightline.setAttribute("x1",currect.getAttribute("x")/1+currect.getAttribute("width")/1-0.1);
		   rightline.setAttribute("y1",currect.getAttribute("y")/1+currect.getAttribute("height")/2-0.1);
		...
		if(rightline2.size()>1){
		 for(var i=0;i<rightline2.size();i++){
		  rightline=rightline2.get(i);
		  rightline.setAttribute("x2",currect.getAttribute("x")/1+currect.getAttribute("width")/1-0.1);
		  rightline.setAttribute("y2",currect.getAttribute("y")/1+currect.getAttribute("height")/2-0.1);
		   $("circle[cx='"+(rightcircle.attr("cx")/1-0.1)+"'][cy='"+(rightcircle.attr("cy")/1-0.1)+"']").attr("cx",currect.getAttribute("x")/1+currect.getAttribute("width")/1-0.1).attr("cy",currect.getAttribute("y")/1+currect.getAttribute("height")/2-0.1);
...
if(curline!=null){
curline.attr("marker-end","url(#arrow)");
}
...
if(linerightcircle.size()==0){
  line1=$("line[x1='"+d3.select(this).attr("x2")+"'][y1='"+d3.select(this).attr("y2")+"']");
  if(line1.size()>1){
...
svg.append("line")
 ...
 svg.append("line")
			 ...
			 .attr("y2",75)
			 .attr("stroke","#666")
			 .attr("stroke-width",2).attr("class","linestart")
    ...
    //判断直线尾部连接点是否与方块上边的连接点最近
      ...
      curline.attr("x2",data.getAttribute("x")/1+data.getAttribute("width")/2-0.1);
        ...
        svg.append("line").attr("x1",curline.attr("x1")/1)
			 ...
			 .attr("stroke","#666")
			 .attr("stroke-width",2).attr("class","lineremove").call(dragline);
